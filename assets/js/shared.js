...
try{(function(){
  ...
  function mergeTranslationOverrides(map){
    ...
    if(changed&&typeof applyTranslations==='function')applyTranslations();
  }
  function mergeTranslationOverridesLang(map){
    ...
    Object.keys(map).forEach(key=>{
      const pair=map[key];
      if(pair.sv){translations.sv[key]=pair.sv;changed=true;}
      if(pair.en){translations.en[key]=pair.en;changed=true;}
    });
    if(changed&&typeof applyTranslations==='function')applyTranslations();
  }
...
(function(){
  var FINDER_SV = [
    {title:'NYBÖRJARE',pill:'Nybörjarpass',bio:'Perfekt start. Vi går igenom grunderna tillsammans, inga förkunskaper behövs.',img:'pass-nyborjare.jpg',route:'vara-pass.html#tl-0',cta:'Se pass →'},
    {title:'FORTSÄTTNING',pill:'Fortsättningspass',bio:'Boka ett provpass så placerar tränaren dig i rätt grupp utifrån din erfarenhet.',img:'pass-fortsattning.jpg',route:'vara-pass.html#tl-1',cta:'Se pass →'},
    ...
    {title:'ALLA NIVÅER',pill:'Boxning, Fyspass, Morgon/Lunch',bio:'Drop-in träning för alla nivåer, ingen tävling. Boxning, Fyspass och Morgon/Lunch passar alla lika bra.',img:'pass-alla-nivaer.jpg',route:'vara-pass.html#tl-4',cta:'Se pass →'},
  ...
  var FINDER_EN = [
    {title:'BEGINNER',pill:'Beginner class',bio:'The perfect start. We go through the fundamentals together, no experience needed.',img:'pass-nyborjare.jpg',route:'vara-pass.html#tl-0',cta:'View class →'},
    {title:'INTERMEDIATE',pill:'Intermediate class',bio:'Book a trial class and the coach will place you in the right group based on your experience.',img:'pass-fortsattning.jpg',route:'vara-pass.html#tl-1',cta:'View class →'},
    ...
    {title:'ALL LEVELS',pill:'Boxing, Fitness Class, Morning/Lunch',bio:'Drop-in training for all levels, no competition. Boxing, Fitness Class and Morning/Lunch all fit equally well.',img:'pass-alla-nivaer.jpg',route:'vara-pass.html#tl-4',cta:'View class →'},
...
// ── IMAGE BAND: fade text in + settle zoom when scrolled into view ──
(function(){
  var band = document.querySelector('.img-band');
  if (!band) return;
  if (!('IntersectionObserver' in window)) { band.classList.add('in-view'); return; }
  var io = new IntersectionObserver(function(entries){
    entries.forEach(function(e){ if (e.isIntersecting) { band.classList.add('in-view'); io.disconnect(); } });
  }, {threshold:0.3});
  io.observe(band);
})();
