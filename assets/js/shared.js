...
async function loadScheduleTeaser(){
  ...
  const toggleEl=document.getElementById('scheduleTeaserToggle');
  if(!el)return;
  ...
  function renderRows(rows){
    if(!rows.length){
      el.innerHTML='<p class="schedule-teaser-empty">Inga pass hittades.</p>';
      return;
    }
    el.innerHTML=rows.map(r=>{
      const pl=splitPassLevel(r['Nivå']||r['Niv\u00e5']||'');
      return `<div class="schedule-teaser-row"><div class="schedule-teaser-row-time">${esc(r.Tid)}</div><div class="schedule-teaser-row-info"><div class="schedule-teaser-row-name">${esc(pl.name)}</div><div class="schedule-teaser-row-level">${esc(pl.level)}</div></div><button type="button" class="schedule-teaser-row-btn">${(translations[currentLang]||{}).sched_teaser_book||'Boka →'}</button></div>`;
    }).join('');
  }
  el.addEventListener('click',e=>{
    if(e.target.closest('.schedule-teaser-row-btn'))openModal();
  });
  try{
    const res=await fetch(CSV_SCHEMA);
    if(!res.ok)throw new Error('bad response');
    ...
    function dayRows(dayName,onlyFuture){
      let list=(byDay[dayName]||[]).filter(r=>!isCancelled(r));
      if(onlyFuture)list=list.filter(r=>{const mins=timeToMinutes(r.Tid);return mins===null||mins>=nowMinutes;});
      return list.slice().sort((a,b)=>(a.Tid||'').localeCompare(b.Tid||'')).slice(0,5);
    }
    const rowsByWhen={today:dayRows(todayName,true),tomorrow:dayRows(tomorrowName,false)};
    let current='today';
    renderRows(rowsByWhen.today);
    if(!rowsByWhen.today.length&&rowsByWhen.tomorrow.length&&toggleEl){
      current='tomorrow';
      renderRows(rowsByWhen.tomorrow);
      toggleEl.querySelectorAll('.schedule-teaser-toggle-btn').forEach(b=>b.classList.toggle('active',b.dataset.when==='tomorrow'));
    }
    if(toggleEl){
      toggleEl.addEventListener('click',e=>{
        const btn=e.target.closest('.schedule-teaser-toggle-btn');
        if(!btn||btn.dataset.when===current)return;
        current=btn.dataset.when;
        toggleEl.querySelectorAll('.schedule-teaser-toggle-btn').forEach(b=>b.classList.toggle('active',b===btn));
        renderRows(rowsByWhen[current]||[]);
      });
    }
  }catch(err){
